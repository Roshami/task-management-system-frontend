  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      >
        ...
        <span className="text-sm font-medium cursor-pointer">Back to Tasks</span>
      </button>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-2 mt-3">
                <span className={`px-3 py-1 rounded-full text-xs font-medium ${statusColors[formData.status].bg} ${statusColors[formData.status].text} border ${statusColors[formData.status].border}`}>
                  {formData.status}
                </span>
                <span className={`px-3 py-1 rounded-full text-xs font-medium ${priorityColors[formData.priority].bg} ${priorityColors[formData.priority].text} border ${priorityColors[formData.priority].border}`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-2">Assigned To</h3>
                  <p className="text-gray-900 font-medium">{formData.assigned_to}</p>
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-2">Start Date</h3>
                    <p className="text-gray-900">{formatDate(formData.start_date)}</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-2">End Date</h3>
                    <p className="text-gray-900">{formatDate(formData.end_date)}</p>
                  </div>
                </div>
              </div>
              ...
              {formData.subtasks.length > 0 && (
                <div>
                  <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-2">Subtasks</h3>
                      ...
                      <div key={index} className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                        ...
                        <div className="space-y-3">
                          <div>
                            ...
                            <p className="text-gray-700">{subtask.subtasks_title}</p>
                          </div>
                          <div>
                            <h5 className="text-xs font-medium text-gray-500 mb-1">Description</h5>
                            <p className="text-gray-700 whitespace-pre-line">{subtask.subtasks_description}</p>
                          </div>
                        </div>
                      </div>
                    ))}
